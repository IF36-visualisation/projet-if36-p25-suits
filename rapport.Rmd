...
---
title: "Analyse de la Diversité des Entreprises selon la Population"
---
...
Nous allons analyser la corrélation entre :
- la **population totale** d'une commune
- et la **diversité des types d'établissements**.
...
library(ggplot2)
library(corrplot)
library(readr) # Charger les données
entreprises <- base_etablissement_par_tranche_effectif
```

```{r}
# On considère les colonnes des différentes tailles d'entreprises
entreprises_prepared <- entreprises %>%
  mutate(
    nb_TPE = E14TS1 + E14TS6,
    nb_PME = E14TS10 + E14TS20 + E14TS50 + E14TS100 + E14TS200,
    nb_GE = E14TS500
  ) %>%
```
...
**Les communes les plus peuplées présentent-elles une diversité plus grande en termes de tailles d'entreprises ?**

Nous nous attendons intuitivement à ce que les grandes communes accueillent **plus de types d'entreprises**.
```{r}
...
```{r}
# Jointure sur le code géographique (CODGEO)
fusion <- entreprises_prepared %>%
...
```
```{r}
region_summary <- fusion %>%
  group_by(REG) %>%
  summarise(
    Total_TPE = sum(nb_TPE, na.rm = TRUE),
    Total_PME = sum(nb_PME, na.rm = TRUE),
    Total_GE = sum(nb_GE, na.rm = TRUE),
    Total_Population = sum(Population, na.rm = TRUE)
  ) %>%
  mutate(
    Diversite = (Total_TPE > 0) + (Total_PME > 0) + (Total_GE>0)
```

# Interprétation

Sur ce graphique, nous observons que :

- Les communes avec une **petite population** ont souvent une **diversité faible** (peu de types d'entreprises différents).
- À mesure que la **population augmente**, la **diversité tend à augmenter** également.
- Toutefois, il existe une certaine variabilité : certaines **communes très peuplées** n'ont pas une **diversité maximale**

```{r}
# Scatter Plot avec régression linéaire
ggplot(data_clean, aes(x = Population, y = Diversity)) +
  geom_point(color = "darkgreen") +
  geom_smooth(method = "lm", se = FALSE, color = "black", linetype = "dashed") +
  labs(
    title = "Population vs Diversité avec Tendance Linéaire",
    x = "Population Totale",
    y = "Diversité des Entreprises"
  ) +
  theme_minimal() +
  scale_x_continuous(labels = scales::comma_format()) +  # Formater l'axe des x
  scale_y_continuous(labels = scales::comma_format())    # Formater l'axe des y

```
# Interprétation

La **droite de régression** montre une **tendance positive** :

- Plus la **population** augmente, plus la **diversité d'établissements** tend à augmenter.

Cependant :

- Le **nuage de points** reste assez dispersé.
- La **corrélation** existe, mais elle n'est pas parfaite.


# À partir de notre exploration

Il existe une **corrélation positive** entre la **taille de la population** et la **diversité des types d'entreprises**.

Cependant, la relation est **modérée** : d'autres éléments (**attractivité économique**, **spécialisation industrielle**, **urbanisation**, etc.) pourraient expliquer la diversité observée.
