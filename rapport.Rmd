...
Nous avons calculé la proportion de chaque mode de cohabitation dans chaque commune, en nous concentrant sur les codes 11 (enfants avec deux parents), 12 (enfants avec un seul parent) et 22 (adultes en couple avec enfants). Ces proportions ont été croisées avec le salaire net moyen par commune.

### Visualisation : répartition salariale selon le mode de cohabitation et carte des salaires moyens par commune


1. **Boxplot** montrant la distribution des salaires nets moyens selon les trois modes de cohabitation (graphique du haut).  
2. **Carte simplifiée** des salaires moyens par commune (graphique du bas), avec localisation des villes principales Paris (P), Lyon (L) et Marseille (M) indiquées par des points et des labels.
...
communes <- st_read(paste0(data_path, "communes.geojson"))

# Identifier la colonne INSEE dans communes
insee_col <- colnames(communes)[str_detect(tolower(colnames(communes)), "insee|code")][1]
...
# Graphique 1 : boxplot salaire moyen selon mode de cohabitation
p1 <- ggplot(moco_salaire, aes(x = factor(MOCO), y = SNHM14, fill = factor(MOCO))) +
  geom_boxplot() +
...
# Joindre communes et salaires
communes_salaire <- communes %>%
  left_join(salaire %>% select(CODGEO, SNHM14), by = setNames("CODGEO", insee_col))

# Coordonnées approximatives des villes clés
villes <- data.frame(
  nom = c("P", "L", "M"),
  lon = c(2.3522, 4.8357, 5.3698),
  lat = c(48.8566, 45.7640, 43.2965)
)

# Graphique 2 : carte simplifiée des salaires moyens avec points et labels pour villes majeures
p2 <- ggplot(communes_salaire) +
  geom_sf(aes(fill = SNHM14), color = NA) +
  scale_fill_gradient(low = "lightblue", high = "darkblue", na.value = "grey90",
                      name = "Salaire moyen (k€)") +
  geom_point(data = villes, aes(x = lon, y = lat), color = "darkblue", size = 3) +
  geom_text(data = villes, aes(x = lon, y = lat, label = nom), color = "black",
            nudge_y = 0.3, size = 4, fontface = "bold") +
  labs(title = "Salaire moyen par commune en France") +
  theme_minimal() +
  theme(
...
# Combiner les graphiques verticalement (p1 au-dessus de p2)
combined_plot <- p1 / p2 + patchwork::plot_layout(heights = c(1, 2))
...
### Analyse de la visualisation
Le boxplot illustre que le salaire net moyen est assez similaire entre les différents modes de cohabitation, avec une médiane proche autour de 13-14 k€, bien que des écarts importants existent avec des valeurs extrêmes dans chaque catégorie.

La carte révèle une concentration des salaires plus élevés dans les grandes agglomérations (Paris, Lyon, Marseille), indépendamment du mode de cohabitation.
Ces résultats suggèrent que, comparé à des facteurs territoriaux et économiques, le mode de cohabitation a un impact limité sur la répartition salariale moyenne.
...
## Question 2 : Quel est l’impact de la densité de population sur les salaires et le nombre de PME ?
