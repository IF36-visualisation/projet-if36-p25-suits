...
# Proportion de grandes entreprises
entreprises_summarise <- entreprises %>%
  mutate(nb_total = rowSums(select(., starts_with("E14TS"))),
         nb_grandes = E14TS100 + E14TS200 + E14TS500,
...
ggplot(data_combine, aes(x = proportion_grandes, y = SNHM14, size = pop_total)) +
  ...
  scale_size(range = c(1, 10), name = "Population totale") +
  ...
  labs(
    ...
    x = "Proportion de grandes entreprises",
...
Pour répondre à cette question, nous avons visualisé la relation entre la proportion de grandes entreprises et le salaire net moyen par commune, avec une ligne de régression pour détecter une tendance.

Le graphique montre que **la plupart des communes ont très peu de grandes entreprises**, et **la pente presque plate de la courbe de régression indique qu’il n’y a pas de lien clair entre leur présence et un salaire plus élevé**. De plus, **les grandes villes (points plus gros) n’ont pas forcément les salaires les plus hauts**, ce qui suggère que **d’autres facteurs influencent le salaire, comme le niveau de vie ou l’activité économique locale**.
