...
## Question : Existe-t-il une relation entre la population totale d'une région et la diversité des tailles d'entreprises qu'elle héberge ?

Les régions les plus denses en population sont-elles aussi les plus diversifiées en termes de types d’entreprises ? Cette analyse vise à explorer la corrélation entre la taille de la population (variable : population totale, E14TS*) et la diversité entrepreneuriale (variable : nombre de classes d’effectifs d’entreprises présentes parmi les 8 classes recensées dans E14TS*). Pour répondre à cette question, nous utiliserons exclusivement un scatter plot, qui permettra de visualiser simplement la relation entre ces deux dimensions. L’hypothèse de départ est que plus la population est élevée, plus la diversité des entreprises sera importante, car une population nombreuse génère une demande variée. Toutefois, nous nous attendons aussi à trouver des cas où certaines régions très denses ne sont pas représentées par les classes d’entreprises de plus grande taille : cela pourrait s’expliquer par une spécialisation économique (par exemple, une forte concentration résidentielle sans présence significative d’industries ou de grandes entreprises). L’analyse devra ainsi permettre d’observer la tendance générale tout en identifiant d’éventuelles exceptions à cette corrélation.

### Visualisation
```{r}
# Chargement interactif des données
entreprises <- read.csv(file.choose(), sep = ",", encoding = "UTF-8")
population <- read.csv("E:/P25/population.csv", sep = ",", encoding = "UTF-8")
# Calcul du nombre de classes non nulles par commune
entreprises_prepared_full <- entreprises %>%
  mutate(
    nb_classes_non_nulles = (E14TS1 > 0) + (E14TS6 > 0) + (E14TS10 > 0) +
                            (E14TS20 > 0) + (E14TS50 > 0) + (E14TS100 > 0) +
                            (E14TS200 > 0) + (E14TS500 > 0)
  ) %>%
  select(CODGEO, REG, nb_classes_non_nulles)

# Assurer que la table population_commune est prête
population_commune <- population %>%
  group_by(CODGEO) %>%
  summarise(Population = sum(NB, na.rm = TRUE))

# Fusion avec les données de population
fusion_full <- entreprises_prepared_full %>%
  inner_join(population_commune, by = "CODGEO")

# Résumé par région
region_summary_full <- fusion_full %>%
  group_by(REG) %>%
  summarise(
    Total_Population = sum(Population, na.rm = TRUE),
    Diversite_moyenne = mean(nb_classes_non_nulles, na.rm = TRUE)
  )

# Scatter plot
ggplot(region_summary_full, aes(x = Total_Population, y = Diversite_moyenne, size = Diversite_moyenne)) +
  geom_point(color = "darkorange", alpha = 0.7) +
  geom_text(aes(label = REG), vjust = -1, hjust = 0.5, size = 3.5) +
  scale_x_log10(labels = comma) +
  labs(
    title = "Population vs Diversité des Classes d'Entreprises",
    x = "Population totale (échelle log)",
    y = "Diversité moyenne des entreprises"
  ) +
  theme_minimal() +
  theme(
...
À partir du graphique obtenu, plusieurs observations peuvent être faites :

- Les régions les moins peuplées (situées à gauche du graphique) tendent à présenter une diversité plus faible de types d'entreprises, souvent entre 1 et 3 classes d'établissements.
- À mesure que la population augmente, on observe globalement une diversification accrue, bien que ce ne soit pas systématique.
- Certaines régions très peuplées (ex : région 11) affichent une diversité plus élevée, tandis que d'autres régions intermédiaires en population (ex : régions 82, 93) présentent une diversité moyenne.
- Le nuage de points montre une tendance générale, mais également une forte dispersion : la population seule n'explique donc pas totalement la diversité économique.
- La transformation logarithmique de l'axe des abscisses (Population totale) améliore considérablement la lisibilité des écarts entre petites et grandes régions.

Ainsi, même si la population semble être un facteur favorisant la diversité, d'autres éléments (économie locale, spécialisation sectorielle, urbanisation) doivent aussi être pris en compte pour expliquer les écarts observés.
